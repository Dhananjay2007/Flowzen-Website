...
2. **Analytics**: No tracking or analytics (e.g., Google Analytics) integrated yet.
   - *Recommendation*: Add a tracking ID once the production domain is live.

3. **Security Headers**: CSP is implemented in `index.html`, but server-side headers (HSTS, etc.) should be configured at the hosting level (e.g., Vercel/Netlify/Nginx configuration).
